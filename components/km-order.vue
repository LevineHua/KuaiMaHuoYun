<template>
	<view class="km_order" @click="seeOrder()">
		<view class="order_info">
			...
			<template v-if="userType==1">	<!-- ((status>2 && status!=6) && userType==0) || userType==1 -->
		...
		<view class="order_status">
			...
			<template v-if="status==2">
				<view class="status" style="margin-bottom: 50upx;">审核未通过</view>
			</template>
		...
		<view class="order_make">
			...
			<template v-if="(userType==0 && status==5)|| (status==5 && userType==1) || (userType==0 && status==6)">
				<view>
					<view class="order_del" @tap="delOrder()" :hover-stop-propagation="propagations">删除</view>
